...
3. The migration enables RLS and revokes anonymous/authenticated direct access on portfolios/trades. Current access-code APIs use service_role; verify no out-of-repository client depends on the old permissive tables. RPCs are granted only to service_role with fixed search paths. Creation/execution enter a dedicated non-login writer role; other RPCs retain invoker security. Direct legacy monetary writes are fenced.
...
Release authorization and execution evidence are tracked in PAPER_TARGET_RELEASE.md. Existing portfolios are preserved; release smoke tests use a separate synthetic paper account.
